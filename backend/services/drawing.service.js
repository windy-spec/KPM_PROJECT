...
class DrawingService {
  ...
  async createDrawing(data) {
    const { product_id, drawing_name, drawing_type, main_image_url, blueprint_image_url, html_code, scale_ratio, parts } = data;
  ...
  async updateDrawing(drawingId, data) {
    const { drawing_name, drawing_type, main_image_url, blueprint_image_url, html_code, scale_ratio, is_active, parts } = data;

    return await prisma.$transaction(async (tx) => {
      // Cập nhật Master
      const master = await tx.product_drawings.update({
        where: { id: drawingId },
        data: { drawing_name, drawing_type, main_image_url, blueprint_image_url, html_code, scale_ratio, is_active }
      });
        ...
        if (parts.length > 0) {
          await tx.drawing_parts.createMany({
            data: parts.map(p => ({
              ...
              part_blueprint_image_url: p.part_blueprint_image_url,
