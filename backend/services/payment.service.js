...
class PaymentService {
  ...
  async createMomoPayment(
    userId,
    { quotation_id, order_id, is_deposit, is_phase_2 },
  ) {
    let amount = 0;
    let title = "";
    ...
    if (quotation_id) {
      ...
      amount = this.resolveOrderPaymentAmount(order, {
        is_deposit,
        is_phase_2,
      });
      title = order.order_code || order_id;
      if (is_phase_2) {
        title = `Thanh toan dot 2 don hang ${title}`;
      } else if (is_deposit && order.deposit_amount) {
  ...
  async createCashPayment(
    ...
    if (order_id) {
      if (is_phase_2) {
        ...
        const phase2Amount =
          Number(freshOrder.total_amount) -
          Number(freshOrder.deposit_amount || 0);
        ...
        if (global.io) {
          ...
          global.io.to("room_admin").emit("orderStatusUpdated", {
            orderId: order_id,
            status: "completed",
          });
          global.io.to("room_warehouse").emit("orderStatusUpdated", {
            orderId: order_id,
            status: "completed",
          });
        }
        ...
        if (!isDepositRequired) {
          ...
          if (global.io) {
            global.io.to("room_admin").emit("new_production_request", {
              message: "Có lệnh sản xuất mới bằng Tiền mặt!",
            });
            global.io.to(`room_user_${userId}`).emit("orderStatusUpdated", {
              orderId: order_id,
              status: "pending",
      ...
      if (order) {
        if (is_phase_2) {
          const phase2Amount =
            Number(order.total_amount) - Number(order.deposit_amount || 0);
          await prisma.orders.update({
            where: { id: order.id },
            data: { production_status: "completed" },
          });
  ...
  async handleMomoWebhook(ipnData) {
    ...
    if (Number(resultCode) === 0) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (transaction.quotation_id) {
          ...
          if (order) {
            ...
            const newDepositAmount = isPhase2
              ? order.total_amount
              : order.deposit_amount;
            const invoiceType = isPhase2
              ? "PHASE_2"
              : isDepositPayment
                ? "DEPOSIT"
                : "TOTAL";
            const invoiceAmount = isPhase2
              ? Number(order.total_amount) - Number(order.deposit_amount || 0)
              : amount;
            ...
            await tx.orders.update({
              ...
              data: {
                ...
                is_deposit_paid: isDepositPayment ? true : false,
          ...
          const orderObj = await tx.orders.findUnique({
          ...
          const invoiceType = isPhase2
            ...
            : isDepositPayment
              ...
              : "TOTAL";
          const invoiceAmount = isPhase2
            ? Number(orderObj.total_amount) -
              Number(orderObj.deposit_amount || 0)
            : amount;
          const newDepositAmount = isPhase2 ? orderObj.total_amount : orderObj.deposit_amount;
      ...
      if (global.io)
        global.io.to("room_admin").emit("new_production_request", {
          message: "Có lệnh sản xuất mới đã thanh toán MoMo!",
        });
      console.log(
  ...
  async createVnpayPayment(
    userId,
    { quotation_id, order_id, ipAddr, is_deposit, is_phase_2 },
  ) {
    let amount = 0;
    let title = "";
  ...
  async handleVnpayIpn(vnp_Params) {
    ...
    if (secureHash === signed) {
      ...
      if (rspCode === "00") {
          ...
          if (transaction.quotation_id) {
            ...
            if (order) {
              ...
              const invoiceAmount = isPhase2
                ? Number(order.total_amount) - Number(order.deposit_amount || 0)
                : amount;
              const newDepositAmount = isPhase2 ? order.total_amount : order.deposit_amount;
              await tx.orders.update({
                where: { id: order.id },
                data: {
                  production_status: newStatus,
        ...
        if (global.io)
          global.io.to("room_admin").emit("new_production_request", {
            message: "Có lệnh sản xuất mới đã thanh toán VNPAY!",
          });
        console.log(
