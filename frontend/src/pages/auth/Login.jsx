...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const normalizedRole =
          ...
          (Array.isArray(user?.roles) &&
            (user.roles[0]?.role_name || user.roles[0]?.name)) ||
          "";
      ...
      if (/tài khoản không tồn tại|không tồn tại|username/i.test(message)) {
        ...
        /mật khẩu|password|chưa được xác thực email|xác thực email/i.test(
          message,
        )
      ) {
  ...
  // =========================================================
  // 2. THÊM HÀM XỬ LÝ ĐĂNG NHẬP GOOGLE VÀO ĐÂY
  // =========================================================
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      if (response.data.success) {
        ...
        setSuccessMessage("Xác thực Google thành công! Đang vào hệ thống...");
        setCountdown(2);
        setShowSuccessOverlay(true);
            ...
            <div className="relative w-full max-w-md overflow-hidden rounded-[28px] border border-emerald-200 bg-white shadow-[0_30px_100px_rgba(15,23,42,0.25)]">
              ...
              <div className="p-8 sm:p-10 text-center">
                ...
                <div className="mt-8 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
                  Hệ thống sẽ tự động chuyển sang trang chủ sau {countdown}{" "}
                  giây.
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 p-8 md:p-14 flex flex-col justify-center">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex items-center gap-2.5 px-1 mt-1">
                <input
                  type="checkbox"
                  id="remember"
                  className="w-4.5 h-4.5 accent-primary cursor-pointer rounded border-outline-variant"
                />
                <label
                  htmlFor="remember"
            ...
            <div className="mt-10 flex flex-col gap-5">
              <div className="relative flex items-center justify-center">
                ...
                <span className="relative px-4 bg-white text-[9px] text-on-surface-variant font-black uppercase tracking-[0.3em]">
                  Hoặc dùng
                </span>
              </div>

              {/* 3. NÚT GOOGLE XỊN SÒ (thay cho nút cũ) */}
              <div className="flex justify-center w-full">
                <GoogleLogin
                  onSuccess={handleGoogleSuccess}
                  onError={() => {
                    setErrorMessage("Hủy đăng nhập Google hoặc có lỗi xảy ra!");
                  }}
                  useOneTap
                  theme="outline"
                  size="large"
