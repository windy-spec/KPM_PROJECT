        ...
        <div className="absolute inset-0 pointer-events-none z-10 p-4 md:p-6 flex flex-col justify-between">
          ...
          <div className="flex justify-between items-start pointer-events-auto">
            <div className="flex items-center gap-3">
              ...
              <Link
                to="/"
                className="flex items-center gap-2 group bg-white/80 backdrop-blur-md px-3 py-2 rounded-xl shadow-sm border border-white/50 hover:bg-white transition-all"
              >
                ...
                <div className="hidden sm:block">
                  <div className="text-sm font-black text-on-surface leading-tight group-hover:text-primary transition-colors">
                    KPM Industry
                  </div>
                  <div className="text-[10px] text-on-surface-variant font-medium leading-tight">
                    Giải pháp Toàn diện
                  </div>
                </div>
              </Link>
            ...
            <div className="flex flex-col items-end gap-3 pointer-events-none">
              ...
              <div
                className={`pointer-events-auto bg-white/95 backdrop-blur-xl p-4 md:p-5 rounded-2xl shadow-2xl border border-white/40 w-[280px] md:w-[320px] transition-all duration-300 origin-top-right flex flex-col max-h-[75vh] ${infoOpen ? "scale-100 opacity-100" : "scale-95 opacity-0 pointer-events-none"}`}
              >
                ...
                <h1 className="text-base md:text-lg font-black text-on-surface mb-1 leading-tight shrink-0">
                  {drawing?.products?.product_name ||
                    drawing?.drawing_name ||
                    "Mô hình Sản phẩm"}
                </h1>
                <p className="text-xs text-on-surface-variant mb-4 font-medium shrink-0">
                  Mã:{" "}
                  {drawing?.products?.product_code ||
                    drawing?.product_code ||
                    "N/A"}
                </p>

                {/* --- DANH SÁCH LINH KIỆN --- */}
                {drawing?.drawing_parts && drawing.drawing_parts.length > 0 && (
                  <div className="mb-4 overflow-y-auto pr-2 flex-1">
                    <div className="text-[10px] text-on-surface-variant/70 font-bold uppercase mb-2 sticky top-0 bg-white/95 py-1 z-10">
                      Chi tiết linh kiện ({drawing.drawing_parts.length})
                    </div>
                    <ul className="space-y-2">
                      {drawing.drawing_parts.map((part) => (
                        <li
                          key={part.id}
                          className="text-xs text-on-surface font-medium flex items-start gap-2 leading-tight"
                        >
                          <span className="text-primary font-bold mt-[1px]">
                            •
                          </span>
                ...
                <div className="pt-3 border-t border-outline-variant/30 flex items-center justify-between shrink-0 mt-auto">
                  <div>
                    <div className="text-[10px] text-on-surface-variant/70 font-bold uppercase mb-0.5">
                      Đơn vị thiết kế
                    </div>
