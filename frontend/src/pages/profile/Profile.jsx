...
const ProfileDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState(currentPanel);
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    setSearchParams({ panel: tab }); // Đẩy URL mới lên thanh địa chỉ
  };
  return (
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-on-surface-variant font-medium">
                Quản lý yêu cầu, báo giá và thông tin cá nhân của bạn
              </p>
            </div>
            ...
            <div className="flex overflow-x-auto border-b border-outline-variant/50 hide-scrollbar pt-2 px-2 gap-1">
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 text-sm font-bold rounded-t-xl transition-all ${
                  activeTab === "overview"
                    ? "bg-white text-primary border-t border-x border-outline-variant/50 border-b-2 border-b-primary shadow-[0_-4px_10px_-5px_rgba(0,0,0,0.05)]"
              ...
              <button
                onClick={() => handleTabChange("ai_history")}
                className={`flex items-center gap-2 px-6 py-3 text-sm font-bold rounded-t-xl transition-all ${
