...
import { User, Mail, Phone, MapPin, ShieldCheck, Edit3, Save, MessageCircle, Lock } from 'lucide-react';
import { authService } from '../../services/auth.service';

const ProfileTab = ({ user, setUser, isEditing, setIsEditing, message, setMessage, errorMessage, setErrorMessage, setLoading, loading }) => {
      ...
      <div className="lg:col-span-2">
        <div className="bg-white border border-outline-variant rounded-3xl p-8 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-black text-on-surface uppercase italic">Thông tin cá nhân</h3>
            {!isEditing ? (
              <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-4 py-2 bg-white border border-outline-variant text-primary text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-primary hover:text-white transition-all shadow-sm">
                <Edit3 className="w-3 h-3" /> Cập nhật
              </button>
            ) : (
              <div className="flex items-center gap-2">
                <button onClick={() => setIsEditing(false)} className="px-4 py-2 bg-surface-container text-on-surface-variant text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-outline-variant transition-all">Hủy</button>
                <button type="submit" form="profile-form" disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-primary text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-primary-container transition-all shadow-lg shadow-primary/20">
                  <Save className="w-3 h-3" /> {loading ? "Đang lưu..." : "Lưu thay đổi"}
                </button>
              </div>
            )}
          </div>
