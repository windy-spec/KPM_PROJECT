...
import { 
  Search, ShoppingCart, User, Factory, LogIn, 
  UserPlus, LogOut, Settings, Package, History 
} from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <header className="bg-white border-b border-outline-variant w-full z-50 sticky top-0 shadow-sm">
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-2">
                ...
                <div className="relative group">
                  ...
                  {/* Dropdown Menu (Giữ nguyên logic cũ nhưng tinh chỉnh vị trí) */}
                  <div className="absolute right-0 top-full pt-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="w-60 bg-white border border-outline-variant rounded-2xl shadow-2xl overflow-hidden">
                      ...
                      <div className="p-2">
                        <Link to="/profile" className="flex items-center gap-3 px-3 py-2 text-sm font-bold text-on-surface-variant hover:text-primary hover:bg-primary/5 rounded-lg transition-colors">
                          <User className="w-4 h-4" /> Hồ sơ cá nhân
                        </Link>
                        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-black text-error hover:bg-red-50 rounded-lg transition-colors mt-1">
              ...
              /* Giao diện Khi chưa đăng nhập (Giữ nguyên hoặc tinh chỉnh bo góc) */
              <div className="flex items-center gap-2">
                ...
                <Link to="/register" className="px-5 py-2.5 bg-primary text-on-primary text-xs font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 hover:bg-primary-container transition-all active:scale-95">
                  Đăng ký
                </Link>
              </div>
            )}
